...
It is based on data recovery from XFS and BTRFS file systems, which I have daily drived for quite extensively.

**For those who don't know about file systems (FS) in general** is file systems are the type of systems that the user use to organise data inside the storage.
...
## What is the need for xfs or btrfs when there are pre-existing file systems ?
...
1. **XFS** (1994):
    ...
    - **Features**:
        - **Journalling**: XFS uses journal (specific file) to log metadata changes before they are committed.
        This ensures consistency and quick recovery in the event of a crash or power failure.
        - **CoW (Copy on Write)**: As the name suggests, copy on wite is a feature that makes a copy of the file which is currently being written.
...
**NOTE**: All the features given here are not everything, but the  most important ones which make them easily distinguishible from other file systems.
